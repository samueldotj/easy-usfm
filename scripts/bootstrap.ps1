...
function Install-Pkg ($id, $label, $probe) {
    ...
    & winget install --id $id --source winget --exact --silent `
...
$havePython = (Test-Tool 'py') -or (Test-Tool 'python3') -or (Test-Tool 'python')
if ($havePython) {
    ...
    if (-not $SkipRust) {
        ...
        if (Install-Pkg 'Rustlang.Rustup' 'rustup' 'rustup') {
            ...
            if (Test-Tool 'rustup') {
                ...
                & rustup target add wasm32-unknown-unknown 2>&1 | Out-String | Write-Verbose
            ...
            if (-not $SkipJust -and -not (Test-Tool 'just') -and (Test-Tool 'cargo')) {
                ...
                & cargo install just 2>&1 | Out-String | Write-Verbose
                Update-SessionPath
...
foreach ($c in @('py -3', 'python3', 'python')) {
    ...
    if (Test-Tool $exe) {
        try {
            ...
            if ($LASTEXITCODE -eq 0) { $python = $c; Write-Ok "python  $c  ($v)"; break }
        } catch { }
    }
}
