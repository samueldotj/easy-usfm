...
- **A correctly closed `\esb` block reports two errors.** `\esb … \esbe` is the one paragraph-level construct in USFM 3.1 whose closing marker is a separate marker rather than a `*` form, and the parser does not model it: given a well-formed block it emits both `UnclosedAtEof` ("still open at end of file") and `StrayCloseMarker` on the `\esbe` — two claims that contradict each other, since the second points at the thing the first says is missing. A study Bible carries hundreds of sidebars, so it is two errors per note across the file. Corrected in `backend::diagnostics::without_false_sidebar_pairs`, which tells the real faults apart by their spans; **worth raising upstream**, and the fix belongs there in the end.
- **Text after a `|` outside a character marker is discarded.** Every `|` is read as the start of an attribute block, and when the marker cannot carry attributes the bar and everything after it is dropped. In Indic scripts that is not markup — `|` is the danda, sentence-ending punctuation, typed as an ASCII bar rather than U+0964 — so this loses Scripture on 14 of the 200 corpus files. Restored in `backend::pipes`, and reconciled in the oracle with the cost named there. **Also worth raising upstream.**
