...
- **`source_map` is a parallel tree**, not spans on nodes — *"AST nodes do not carry spans."* Click-to-source and diagnostic placement walk two trees in lockstep. Expect a zipper or an index; `to_usj(UsjOptions { include_spans: true })` is an alternative path.
